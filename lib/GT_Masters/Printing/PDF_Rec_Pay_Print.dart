...
class Rec_Pay_Print extends StatefulWidget {
  var Parm_Id;
  var printType;
  Rec_Pay_Print({this.Parm_Id,this.printType});

  @override
  _Rec_Pay_PrintState createState() => _Rec_Pay_PrintState();
}

class _Rec_Pay_PrintState extends State<Rec_Pay_Print> {
...
//------------------for appbar------------
  ...
  GetdataPrint(url) async {
    ...
    try {
      final tagsJson =
      await http.get("${Env.baseUrl}$url", headers: {
        //Soheader //SalesHeaders
        "Authorization": user.user["token"],
      });
