...
class _Master_DistrictState extends State<Master_District> {
  ...
  int Mstr_State_Id=null;
var Edit_Id=null;

  var District_List=[];

  static List<DistrictList> _districtlist = new List<DistrictList>();

  static List<StateList> _statelist = new List<StateList>();
// //----------------------------------
  void initState() {
    setState(() {
...
//------- -------------------------
  GetDistrict()async{
// print("in GetArea");
    final json= await http.get("${Env.baseUrl}MDistricts",
        headers: {
...
//print(json.body.toString());
  ...
  GetState()async{
// print("in GetArea");
    final _json= await http.get("${Env.baseUrl}mstates",
        headers: {
          "Authorization": user.user["token"],
        }
    ) ;
...
class DistrictList {
  ...
  DistrictList({this.id,this.mdName,this.mdStateId,this.msName});
