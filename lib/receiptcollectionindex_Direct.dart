...
import 'package:flutter_app/updatereceiptcollection.dart';
import 'package:flutter_app/urlEnvironment/urlEnvironment.dart';
import 'package:intl/intl.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:http/http.dart' as http;

class ReceiptCollectionIndex_Direct extends StatefulWidget {


  dynamic passvalue;

  ReceiptCollectionIndex_Direct({this.passvalue});

  @override
  _ReceiptCollectionIndex_DirectState createState() => _ReceiptCollectionIndex_DirectState();
}

class _ReceiptCollectionIndex_DirectState extends State<ReceiptCollectionIndex_Direct> {
  dynamic branchName;
  dynamic userName;
  dynamic data;
  ...
  Future<String> getCustomer(dynamic id) async {
    try {
      final response = await http
          .get("${Env.baseUrl}MLedgerHeads/${id.toString()}", headers: {
...
//        users=loadUsers(s.toString());
  ...
  getIndexReceiptCollectionorg() async {
    ...
    var res = await http.get("${Env.baseUrl}MobSmrctCollectionHeaders",
  ...
  getIndexReceiptCollection(id)async{
    ...
    var res = await http.get("${Env.baseUrl}TaccReceiptHeaders",
        headers: {
          'accept': 'application/json',
          'content-type': 'application/json',
          'Authorization': user.user["token"],
                                    ...
                                    cells: [
                                      ...
                                      visibilityTableRow? DataCell(
                                        GestureDetector(
                                          onTap: () {},
                                          child: Text(
                                              '${itemRow['rvhVoucherNumber'].toString()}'),
                                        ),
                                        showEditIcon: false,
                                        placeholder: false,
                                      )
                                      ...
                                      DataCell(
                                          Text(DateFormat('MMM dd , yyyy').format(DateTime.parse(itemRow['rvhVoucherDate']))
                                          .toString()),
                                        showEditIcon: false,
                                        placeholder: false,
                                      ),
                                      DataCell(
                                        Container(
                                          alignment:Alignment.centerRight,
                                          child: Text(DateFormat('MMM dd , yyyy').format(DateTime.parse(itemRow['rvhEntryDate']))
                                              .toString()),
                                        ),
                                        showEditIcon: false,
                                        placeholder: false,
                                          ...
                                          child: GestureDetector(
                                              onTap: () {
                                                ...
                                                Navigator.push(
                                                    ...
                                                    MaterialPageRoute(
                                                        builder: (context) =>
                                                            ReceiptCollections_Direct(passname:"..",passvalue:itemRow['id'],
                                                                )));
                                              },
                                              child:
                                              Icon(Icons.edit)),
