...
import 'package:flutter_app/CustomerDetails.dart';
...
import 'package:flutter_app/itemBill.dart';
import 'package:flutter_app/itemBill_Index.dart';
import 'package:flutter_app/itemCreate.dart';
import 'package:flutter_app/newtestpage.dart';
import 'package:flutter_app/printing.dart';
import 'package:flutter_app/qr_BarCode_Reader.dart';
...
import 'package:flutter_app/workAssignMaster.dart';
...
//    debugPrint('${position}');
      ...
      double totalDistance = calculateDistance(position.latitude,
...
//    this.position});
      ...
      print(placeMark[0].subAdministrativeArea);
      location =
          '${placeMark[0].locality}  ${placeMark[0].subAdministrativeArea}';
      print("longitude:");
      print(location);
...
//        });
  ...
  getCustomerRegistered(dynamic ledgerId) async {
    ...
    var res = await http.get(
        "${Env.baseUrl}RelMcustLedgerLocations/${ledgerId.toString()}/1",
  ...
  customerRegisterNow() async {
    ...
    var res = await http.post("${Env.baseUrl}RelMcustLedgerLocations",
        headers: {
  ...
  regCustomerIn() async {
    ...
    var res = await http.post("${Env.baseUrl}TcustomerVisitHistories",
        headers: {
          'accept': 'application/json',
          'content-type': 'application/json',
          'Authorization': user.user['token']
...
//     }
    ...
    var res = await http.put(
        "${Env.baseUrl}TcustomerVisitHistories/${VisitInId.toString()}/1/1",
        headers: {
          'accept': 'application/json',
          'content-type': 'application/json',
          'Authorization': user.user['token'],
...
///------------------------------------------------------------------------------
  ...
  Future<bool> _onBackPressed() {
    Navigator.pushReplacement(
      ...
      AppTyp=="RMS" ?  Rms_Homes(): SalesManHome()
      ),
    );
  }
