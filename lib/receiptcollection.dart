...
import 'package:flutter_app/models/receiptAdditem.dart';
import 'package:flutter_app/models/receiptCollection.dart';
...
class ReceiptCollections extends StatefulWidget {

  int passvalue;
  dynamic passname;
   ReceiptCollections({this.passvalue,this.passname});



  @override
  _ReceiptCollectionState createState() => _ReceiptCollectionState();
}

@override
class _ReceiptCollectionState extends State<ReceiptCollections> {
...
//  var _dobController = TextEditingController();
  ...
  List<ReceiptAdd> customerReceipts = new List<ReceiptAdd>();
  List<ReceiptAdd> finalCustomerReceipts = new List<ReceiptAdd>();

  double longitude;
  double latitude;

  ReceiptAdd customerReceipt;
...
//      maxProgress: 100.0,
    ...
    var res = await dio.post(
      "${Env.baseUrl}MobSmrctCollectionHeaders",
      data: form,
  ...
  printfunction(datas) async{
    try {
     ...
     //print(jsonEncode(datas));
       dataForTicket = await datas;
     // print(dataForTicket.toString());
     // print(dataForTicket["mobSmrctCollectionDetails"].toString());


      Timer(Duration(milliseconds: 1), () async {
        blutoothprinting();
        //_ticket(PaperSize.mm58);200
      });
    } catch (e) {
