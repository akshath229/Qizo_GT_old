...
import 'package:flutter_app/GT_Masters/Printing/PDF_Rec_Pay_Print.dart';
...
import 'package:flutter_app/salesreturn.dart';
...
class Payment_Voucher_Direct extends StatefulWidget {

  int passvalue;
  dynamic passname;
  Payment_Voucher_Direct({this.passvalue,this.passname});



  @override
  _Payment_Voucher_DirectState createState() => _Payment_Voucher_DirectState();
}

@override
class _Payment_Voucher_DirectState extends State<Payment_Voucher_Direct> {
...
//  var _dobController = TextEditingController();
  ...
  TextEditingController Cheque_DateController = new TextEditingController();
   ...
   List<Receipt_Payment_Add> customerReceipts = new List<Receipt_Payment_Add>();
  List<Receipt_Payment_Add> finalCustomerReceipts = new List<Receipt_Payment_Add>();
  ...
  Receipt_Payment_Add customerReceipt;
...
var BtnName="Save";
  ...
  GetVchnum()async{
    ...
    try {
      final tagsJson =
      await http.get("${Env.baseUrl}getSettings/1/paymentvoucher", headers: {
...
print(Payment_Type_id.toString());
    ...
    var req = {
      "pvhVoucherNumber": VchNum,
      "pvhVoucherDate": RptDateTime,
      "pvhHeadOfAccount": Payment_Type_id,
      "apNarration": remarksController.text,
      "pvhEntryDate": RptDateTime,
      "pvhUserId": userId,
      "pvhBranchId": branchId,
      "taccPaymentDetails": [

        for(int i=0;i<receiptCollection.length;i++)
          {

            "pdLedgerAccount":receiptCollection[i].lgr_id,
            "pdAmount": receiptCollection[i].rdAmount,
            "pdSingleRemarks": receiptCollection[i].rdRemarks,
            "pdTranSerialNo": i,
            "pdMethodOfAdjust": null,
            "pdAdvanceRefNo": null,
            "pdChequeNo": receiptCollection[i].Cheque_No,
            "pdChequeDate": receiptCollection[i].Cheque_Date,

          }],
      "taccPaymentAgainstRef": []
    };
...
//     );
    ...
    var res = await http.post("${Env.baseUrl}taccpaymentheaders",
        headers: {
    ...
    if (res.statusCode == 200 ||
      ...
      print(res.body);
      var responsedata=await json.decode(res.body);
      ...
      setState(() {
        ...
        showDialog(barrierDismissible: false,
            ...
            builder: (context) =>
                AlertDialog(
                  title: Text("Payment Added Success"),
                  content: Text("$customerName"),
                ));
        Timer(Duration(seconds: 2), () {
          print("Yeah, this line is printed after 2 seconds");
          Navigator.pop(context);
          var res_id=responsedata['id'];
          printfunction(res_id);
...
//--------------------------Edit part----------------------------------
  ...
  EditBinde(id)async{
    ...
    print("uiouiouioiu");
    print(id.toString());
    var res = await http.get("${Env.baseUrl}taccpaymentheaders/$id",
        headers: {
          'accept': 'application/json',
          'content-type': 'application/json',
          'Authorization': user.user["token"],
...
setState(() {
  RptDateTime=Editresdata['header'][0]['pvhEntryDate'];
  VchNum=Editresdata['header'][0]['pvhVoucherNumber'];
});
  ...
  Detailpart.forEach((element) {
    ...
    customerReceipt = Receipt_Payment_Add(
        slNo: customerReceipts.length + 1,
        receivedAmount: element["pdAmount"],
        remarks: element["pdSingleRemarks"],
  ...
  GetCash()async {
    var res = await http.get("${Env.baseUrl}MLedgerHeads/1/3",
        headers: {
          'accept': 'application/json',
          'content-type': 'application/json',
          'Authorization': user.user["token"],
  ...
  GetBank()async {
    var res = await http.get("${Env.baseUrl}MLedgerHeads/1/4",
        headers: {
          'accept': 'application/json',
          'content-type': 'application/json',
          'Authorization': user.user["token"],
...
//            content: Text("$customerName"),
    ...
    var map = json.encode(req);
    print(map);
    var res = await http.put("${Env.baseUrl}taccpaymentheaders/${widget.passvalue}",
        headers: {
          'accept': 'application/json',
          'content-type': 'application/json',
          'Authorization': user.user["token"],
    ...
    try {
      ...
      Timer(Duration(milliseconds: 1), () async {
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    Rec_Pay_Print(Parm_Id: datas,printType: "paymt")));
      });
    } catch (e) {
...
class Receipt_Payment_Add {
  ...
  Receipt_Payment_Add({this.slNo,this.receivedAmount,this.remarks,
    this.image,this.rdtransactionType,this.LedgerAccount,
    this.LedgerAccount_id,this.Cheque_Date,this.Cheque_No});
