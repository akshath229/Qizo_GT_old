...
class Set_Cust_Location2 extends StatefulWidget {
  @override
  _Set_Cust_Location2State createState() => _Set_Cust_Location2State();
}

class _Set_Cust_Location2State extends State<Set_Cust_Location2> {
...
//        users=loadUsers(s.toString());
  ...
  Get_LocationPermissionSatus()async{
    ...
    if(geolocationStatus.toString().contains("denied")) {
      Rqst_LocationPerinsion();
      return false;
    }else{
...
// print(position.longitude);
  ...
  PostCust_Location(lan,lon,location)async{
    ...
    var res = await http.post("${Env.baseUrl}customerlocations",
        headers: {
  ...
  GetLocation()async{
    ...
    final response =
    await http.get("${Env.baseUrl}customerlocations/$customer_Id/cust", headers: {
      "Authorization": user.user["token"],
    });
    if(response.statusCode==200){
