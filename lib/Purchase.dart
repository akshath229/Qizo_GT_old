...
class Purchase extends StatefulWidget {
  ...
  int passvalue;
  dynamic passname;
  Purchase({this.passvalue,this.passname,this.itemrowdata});
...
//   validation variables
  ...
  static List<purchaseedit> purchaseedt = new List<purchaseedit>();
  static List<purModel> purchase = new List<purModel>();
...
//              content: Text("user data:   " + user.user["token"]),
        ...
        debugPrint(params);
        // return;
        var res = await http.put(Edit_url,
            headers: {
              'accept': 'application/json',
              'content-type': 'application/json',
              'Authorization': user.user["token"],
...
//                          Text("Call"), // text
  ...
  databinding(id) async{
    ...
    try {
      final bindata =
      await http.get("${Env.baseUrl}purchaseHeaders/$id", headers: { //Soheader //purchaseHeaders
        "Authorization": user.user["token"],
      });
...
//--------------------Print part-------------------------------------------
  ...
  GetdataPrint(id) async {
    ...
    try {
      final tagsJson =
      await http.get("${Env.baseUrl}purchaseHeaders/$id", headers: {
  ...
  footerdata() async {
    try {
      ...
      final tagsJson =
      await http.get("${Env.baseUrl}purchaseInvoiceFooters/", headers: {
        "Authorization": user.user["token"],
      });
