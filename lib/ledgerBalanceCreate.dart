...
import 'package:flutter_app/GT_Masters/Models/LedgerGroupsModel.dart';
...
class CreateLedgeBlnc extends StatefulWidget {
  var Parm_id;
  String PageType;
  CreateLedgeBlnc({this.Parm_id,this.PageType});

  @override
  _CreateLedgeBlncState createState() => _CreateLedgeBlncState();
}

class _CreateLedgeBlncState extends State<CreateLedgeBlnc> {
  double TextBoxCurve=10.0;
  AppTheam theam =AppTheam();

  static List<LedgerGrpModel>_ledgerGrpModel=List<LedgerGrpModel>();
  static List<Area> area = new List<Area>();
  static List<StateMaster> state=new List<StateMaster>();
...
List <LedgerHeads> LedgH= List <LedgerHeads>();
...
GetArea()async{
// print("in GetArea");
  final json= await http.get("${Env.baseUrl}mareas",
      headers: {
...
//-----------get State--------------------------
  GetState()async{
  //  print("in GetState");
    final json= await http.get("${Env.baseUrl}mstates",
        headers: {
          "Authorization": user.user["token"],
        }
    ) ;
...
if(ButtonName=="Save") {
  ...
  var result = await http.post("${Env.baseUrl}MLedgerHeads",
    headers: {
...
class StateMaster {
  ...
  StateMaster({this.id,this.msName,this.msStateCode});
