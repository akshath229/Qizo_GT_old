...
import 'package:flutter_app/AQizo_RMS/New_Delivery_App/Del_ItemAddOn.dart';
import 'package:flutter_app/AQizo_RMS/New_Delivery_App/Del_Model/Del_Model_AddonItms.dart';
import 'package:flutter_app/AQizo_RMS/New_Delivery_App/Del_View_And_Print_PerBill.dart';
import 'package:flutter_app/AQizo_RMS/RMS_Print/RMS_SimPrint.dart';
...
//-----------------------------------------
  ...
  ItmBillDetails ItmBLdata;
  Del_Tabledata TDdata;
  dynamic branch;
  //var res;
  dynamic user;
  int branchId;
...
//----------------------------------
  ...
  List<dynamic> PayTypData; // get Paymet type api data
  var PayAmtTypData;
  dynamic UOM = null;
  var UomTableData = {};
  var AddonData;
  ...
  static List<Del_Tabledata> TD = new List<Del_Tabledata>();
  static List<ItemGroup> ItmGrp = new List<ItemGroup>();
  static List<ItemList> ItmLst = new List<ItemList>();
  ...
  static List<ItmBillDetails> IBD = new List<ItmBillDetails>();
  ///-------add on
...
//----------------------getCustomer-----------------------------------------
  getCustomer() async {
    try {
      final response = await http.get("${Env.baseUrl}MLedgerHeads", headers: {
  ...
  IBSave(Detailsdta) async {
    ...
    if(ButtonName=="Save"){
     ...
     // final resSclose = await http.get("${Env.baseUrl}GeneralSettings/1/Getrmssale",
      final resSclose = await http.get("${Env.baseUrl}GeneralSettings",
          headers: {
        "Authorization": user.user["token"],
      }
          );
     ...
     print("save ${Env.baseUrl}SalesHeaders");
      ...
      res = await http.post("${Env.baseUrl}SalesHeaders",
          headers: {
    ...
    else// edit
    {
      ...
      res = await http.put("${Env.baseUrl}SalesHeaders/$EditId",
          headers: {
            'accept': 'application/json',
            'content-type': 'application/json',
            'Authorization': user.user["token"],
...
//------------------------  Payment Completed-------------
  ...
  Get_PerviousBill_Id()async{
    ...
    try {
      ...
      final res =
      await http.get("${Env.baseUrl}SalesHeaders", headers: {
        "Authorization": user.user["token"],
      });
...
class ItemGroup {
  ...
  ItemGroup({this.id, this.igDescription, this.igDiplayImagepath});
...
class ItemList {
  ...
  ItemList({this.id, this.itmName, this.itmImage, this.itmTaxInclusive, this.showAddonItems});
