...
class Sales_Report extends StatefulWidget {
  ...
  @override
  _Sales_Report createState() => _Sales_Report();
}

class _Sales_Report extends State<Sales_Report>
  ...
  var PtypId=null;
  var StockData=[];
  static List<Payment_Type> paymenttype = new List<Payment_Type>();
  ...
  GetSalesRpt()async{
     ...
     print(jsonParse);
    try {
      final res =await cw.post(api:"GtSales/2",body: jsonParse,Token: token);
      if(res!=false) {
        setState(() {
          print(res);
  ...
  GetPaymentTyp()async{
    ...
    try {
      ...
      if(res.statusCode==200) {
        ...
        List<Payment_Type> gd = tagsJson.map((tagsJson) =>
            Payment_Type.fromJson(tagsJson)).toList();
        print("paymenttype : $gd");
        paymenttype = gd;
        // GodownController.text =tagsJson[0]["paymentType"];
...
//    key: scaffoldKey,
        appBar: PreferredSize(
            ...
            child:  Appbarcustomwidget(uname: userName, branch: branchName, pref: pref, title:"Sales Report")
        ),
...
//                            errorBorder:InputBorder.none ,
                            ...
                            labelText:
                            'Paymet Type', // i need to decrease height
                          )),
                      suggestionsBoxDecoration:
                      SuggestionsBoxDecoration(elevation: 90.0),
                      suggestionsCallback: (pattern) {
                        return paymenttype.where((ptyp) =>
                            ptyp.paymentType.toUpperCase().contains(pattern.toUpperCase()));
                      },
                      ...
                      onSuggestionSelected: (suggestion) {
                        print(suggestion.paymentType);
                        print("selected");
                        print(suggestion.id);
                        PaymentTypController.text = suggestion.paymentType;
                        PtypId=suggestion.id;
