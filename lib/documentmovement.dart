...
//  var _myKey = GlobalKey<FormState>();
  ...
  PickedFile pic;
  static List<dynamic> users = new List<dynamic>();
...
//            ));
  ...
  getVoucherNumber() async {
    ...
    var res = await http.get('${Env.baseUrl}getSettings/1/documentmanage',
...
// get Employee Mater account
  getEmployee() async {
    try {
      // http://testcoreapi.qizo.in:809/api/
      final response =
      await http.get("${Env.baseUrl}MemployeeMasters", headers: {
        "Authorization": user.user["token"],
      });
      print(response.statusCode);
      print(response.body);
